...
export class Anomaly {
  protected readonly registrationTrend = [8, 9, 7, 10, 9, 11, 8, 9, 12, 10, 9, 38, 41, 36, 12, 9];
  ...
  protected readonly detectSeries: Series[] = [
    {
      ...
      data: [
        9, 11, 8, 10, 12, 9, 11, 10, 8, 12, 9, 11, 10, 13, 9, 11, 8, 10, 12, 9, 10, 11, 9, 12, 10,
        9, 11, 47, 58, 52, 41, 33, 14, 10, 9, 11, 10, 12, 9, 10,
      ],
    },
    {
      ...
      dashed: true,
      data: [
        24, 24, 23, 24, 25, 24, 24, 24, 23, 25, 24, 24, 24, 26, 24, 24, 23, 24, 25, 24, 24, 24, 24,
        25, 24, 24, 24, 26, 28, 29, 28, 27, 26, 25, 24, 24, 24, 25, 24, 24,
      ],
    },
  ];
  ...
  protected readonly threats: ThreatModel[] = [
    {
      name: 'Rogue UE / IMSI Enumeration',
      signal: 'Registration burst from single origin',
      method: 'Rate threshold + Z-score',
      status: 'triggered',
      hits: 1,
    },
    {
      name: 'IMSI Catcher (Fake gNB)',
      signal: 'Auth without AKA, identity request',
      method: 'NAS sequence FSM',
      status: 'triggered',
      hits: 1,
    },
    {
      name: 'Session Hijacking',
      signal: 'Modify for unknown SMF session',
      method: 'Cross-NF state correlation',
      status: 'armed',
      hits: 0,
    },
    {
      name: 'DoS on AMF',
      signal: 'Registration flood per PLMN/TA',
      method: 'Sliding-window rate limiter',
      status: 'armed',
      hits: 0,
    },
  ];
  ...
  protected async mitigate(a: Alert): Promise<void> {
    ...
    if (!ok) return;
    this.alerts.update((list) => list.map((x) => (x === a ? { ...x, state: 'mitigated' } : x)));
